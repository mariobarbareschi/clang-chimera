...
#define DEBUG_TYPE "mutator_truncadder"
...
bool chimera::adder::MutatorTruncAdder::getMatchedNode(const NodeType &node, DynTypedNode &dynNode) {
...
///         operation
StatementMatcher chimera::adder::MutatorTruncAdder::getStatementMatcher() {
...
bool chimera::adder::MutatorTruncAdder::match(const NodeType &node) {
...
Rewriter &chimera::adder::MutatorTruncAdder::mutate(const NodeType &node, MutatorType type, Rewriter &rw) {
    ...
    if(templDecl != NULL) {
      if (first_call) {
        rw.InsertTextBefore(templDecl->getSourceRange().getBegin(), "#define TRUNC_MAX(x, y, nab)    ((sizeof((x)) < sizeof(y)) ? ((nab) % (sizeof((x))*8)) : ((nab) % (sizeof((y))*8)))\n#define TRUNC_MASK(x, y, nab)   ~((1 << TRUNC_MAX(x,y,nab)) - 1)\n#define TRUNC_ADDER(x, y, nab)  (((x) + (y)) & TRUNC_MASK(x, y, nab))\n");
        first_call = false;
      }

      // Information for the report:
      MutatorTruncAdder::MutationInfo mutationInfo;
      ...
      if (first_call) {
        rw.InsertTextBefore(funDecl->getSourceRange().getBegin(), "#define TRUNC_MAX(x, y, nab)    ((sizeof((x)) < sizeof(y)) ? ((nab) % (sizeof((x))*8)) : ((nab) % (sizeof((y))*8)))\n#define TRUNC_MASK(x, y, nab)   ~((1 << TRUNC_MAX(x,y,nab)) - 1)\n#define TRUNC_ADDER(x, y, nab)  (((x) + (y)) & TRUNC_MASK(x, y, nab))\n");
        first_call = false;
      }
  ...
  do{
    ...
    ::std::string bopReplacement = "TRUNC_ADDER(" + lhsString + ", " + rhsString + ", " + nabId + ")";
...
void chimera::adder::MutatorTruncAdder::onCreatedMutant(const ::std::string &mDir) {
  ...
  while( !(this->mutationsInfo.empty()) ){
    ...
    MutatorTruncAdder::MutationInfo mutationInfo = this->mutationsInfo.back();
